...
def reviewed_objects(collection, side):
    ...
    return [
        ...
        and (
            ...
            or obj.get('production_glove_articulation_revision') == ARTICULATION_REVISION
        )
    ]
...
def main():
    ...
    if armature is None or camera is None or home is None or away is None:
        raise RuntimeError('The private articulated-glove review workfile is incomplete.')
    ...
    for variant, visible, hidden, action_frames, views in (
        ...
        for action_name, frame in action_frames.items():
            ...
            if action is None:
                raise RuntimeError(f'Missing articulated-glove review action: {action_name}')
            armature.animation_data.action = action
            scene.frame_set(frame)
            bpy.context.view_layer.update()
            for side in ('Left', 'Right'):
                objects = reviewed_objects(visible, side)
                if len(objects) != 22:
                    raise RuntimeError(
                        f'Expected 22 articulated {variant} {side} glove objects, found {len(objects)}.'
                    )
                ...
                for view in views:
                    ...
                    output = output_dir / (
                        f'production-glove-articulation-{variant}-{side.lower()}-'
    ...
    report = {
        ...
        'outputs': outputs,
        'reviewBoundary': (
            'The close set must prove non-repeating finger plates, visible articulation gaps, '
            'layered palm leather and stalls, shaft compression, stable two-hand contact, and '
            'the absence of floating or penetrating authored details.'
        ),
    }
    output_report.write_text(json.dumps(report, indent=2), encoding='utf-8')
    print('GOON_VNEXT_PRODUCTION_GLOVE_ARTICULATION_REVIEW_RENDERED ' + str(output_report))
