...
def main():
    ...
    if source is None or home is None or away is None or contact is None or armature is None:
        raise RuntimeError('The authored vNext contact workfile is incomplete.')

    action_names = sorted(action.name for action in bpy.data.actions if action.name)
    variants = {}
    for side, collection in [('home', home), ('away', away)]:
        output = output_dir / f'goon-field-player-{side}-contact-v1.glb'
        ...
        variants[side] = {
            ...
            'objects': objects,
            'stickObjects': [name for name in objects if '_Stick_' in name],
            'includesBall': 'GS_Contact_Ball' in objects,
        }
    report = {
        'status': 'contact-candidates-exported',
        'decision': 'not-public-runtime-approved',
        'publicRuntimeAllowed': False,
        'sourceWorkfile': bpy.data.filepath,
        'actionNames': action_names,
        'actionCount': len(action_names),
        'contactFrames': {'receive': 16, 'pass': 16, 'shot': 20},
        'variants': variants,
    }
    output_report.write_text(json.dumps(report, indent=2), encoding='utf-8')
    print('GOON_VNEXT_CONTACT_EXPORTED ' + str(output_report))
