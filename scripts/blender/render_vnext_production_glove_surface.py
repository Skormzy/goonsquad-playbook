...
def surface_objects(collection, side):
    ...
    return [
        ...
        and (
            obj.get('production_glove_fit_revision')
            or obj.get('production_glove_surface_revision') == SURFACE_REVISION
        )
    ]
...
def view_offset(view):
    if view == 'front':
        return Vector((0.0, -0.52, 0.025))
    if view == 'left-three-quarter':
        return Vector((0.37, -0.40, 0.07))
    return Vector((-0.37, -0.40, 0.07))
...
def main():
    ...
    if armature is None or camera is None or home is None or away is None:
        raise RuntimeError('The private production-glove surface workfile is incomplete.')
    ...
    for variant, visible, hidden, action_frames, views in (
        ...
        for action_name, frame in action_frames.items():
            ...
            if action is None:
                raise RuntimeError(f'Missing production-glove review action: {action_name}')
            armature.animation_data.action = action
            scene.frame_set(frame)
            bpy.context.view_layer.update()
            for side in ('Left', 'Right'):
                objects = surface_objects(visible, side)
                if len(objects) != 33:
                    raise RuntimeError(
                        f'Expected 33 {variant} {side} surface objects, found {len(objects)}.'
                    )
                ...
                for view in views:
                    ...
                    output = output_dir / (
                        f'production-glove-surface-{variant}-{side.lower()}-'
    ...
    report = {
        'status': 'rendered-for-private-production-glove-surface-review',
        ...
        'outputs': outputs,
        'reviewBoundary': (
            'The close set must prove a closed palm around the shaft, a credible thumb web, '
            'a tapered cuff-to-sleeve transition, covered hands, left-right fit, and stable '
            'ready, pass, and shot shapes.'
        ),
    }
    output_report.write_text(json.dumps(report, indent=2), encoding='utf-8')
    print('GOON_VNEXT_PRODUCTION_GLOVE_SURFACE_REVIEW_RENDERED ' + str(output_report))
