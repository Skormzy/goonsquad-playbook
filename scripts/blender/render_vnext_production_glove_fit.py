...
def fitted_objects(collection, side):
...
def main():
    ...
    if armature is None or camera is None or home is None or away is None:
        raise RuntimeError('The private fitted-glove review workfile is incomplete.')
    ...
    for variant, visible, hidden, action_frames, views in (
        ...
        for action_name, frame in action_frames.items():
            ...
            if action is None:
                raise RuntimeError(f'Missing fitted-glove review action: {action_name}')
            armature.animation_data.action = action
            scene.frame_set(frame)
            bpy.context.view_layer.update()
            for side in ('Left', 'Right'):
                objects = fitted_objects(visible, side)
                if len(objects) < 30:
                    raise RuntimeError(
                        f'Expected the complete {variant} {side} fitted glove, found {len(objects)} objects.'
                    )
                target = evaluated_bounds_center(objects)
                for view in views:
                    camera.location = target + view_offset(view, side)
                    point_camera(camera, target)
                    output = output_dir / (
                        f'production-glove-fit-{variant}-{side.lower()}-'
    ...
    report = {
        'status': 'rendered-for-private-production-glove-fit-review',
        ...
        'outputs': outputs,
        'reviewBoundary': (
            'The close set must prove manufactured glove volume, closed fingers around the shaft, '
            'covered hands, cuff clearance, left-right fit, and stable ready, pass, and shot shapes.'
        ),
    }
    output_report.write_text(json.dumps(report, indent=2), encoding='utf-8')
    print('GOON_VNEXT_PRODUCTION_GLOVE_FIT_REVIEW_RENDERED ' + str(output_report))
