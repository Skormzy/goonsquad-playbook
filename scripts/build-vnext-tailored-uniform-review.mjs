...
const tag = 'cmu16-ik-tailored-uniform';
const sourceWorkfile = path.join(
  root, 'asset-inbox', 'players', 'vnext',
  'goon-field-player-cmu16-ik-silhouette-audition.blend',
);
...
const evidenceDir = path.join(
  root, 'docs', 'vnext', 'evidence', 'athlete-tailored-uniform-review', 'actions',
);
const reports = {
...
if (!fs.existsSync(sourceWorkfile)) {
  console.error('The private silhouette source workfile is missing.');
  process.exit(1);
}
...
run(blender, [
  ...
  '--python', path.join(root, 'scripts', 'blender', 'refine_vnext_tailored_uniform.py'),
  '--',
  '--output-workfile', outputWorkfile,
  '--output-report', reports.refinement,
], 'tailored uniform authoring');

for (const [label, script, report] of [
  ['tailored silhouette audit', 'audit_vnext_silhouette.py', reports.silhouette],
  ['tailored material audit', 'audit_vnext_materials.py', reports.materials],
  ['tailored topology audit', 'audit_vnext_uniform_topology.py', reports.topology],
  ['tailored deformation audit', 'audit_vnext_uniform_deformation.py', reports.deformation],
]) {
  run(blender, [
...
for (const side of ['home', 'away']) {
  ...
  run(process.execPath, [gltfTransformCli,
    ...
    '--texture-size', '1024',
  ], `${side} tailored transport optimization`);
  fs.copyFileSync(
    optimized,
...
], 'tailored runtime asset audit');

console.log('GOON_VNEXT_TAILORED_UNIFORM_REVIEW_READY');
