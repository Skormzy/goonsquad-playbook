...
const sourceBlend = path.join(root, 'asset-inbox', 'players', 'mocap', 'cmu-35', 'cmu-35-24-source.blend');
const targetBlend = path.join(workDir, 'goon-field-player-cmu-sprint-audition.blend');
const outputBlend = path.join(workDir, 'goon-field-player-cmu-run-audition.blend');
const retargetReport = path.join(workDir, 'cmu-jog-retarget-report.json');
const strideReport = path.join(workDir, 'cmu-jog-stride-report.json');
const renderReport = path.join(workDir, 'cmu-jog-retarget-render-report.json');
const reviewDir = path.join(root, 'docs', 'vnext', 'evidence', 'cmu-jog-retarget-review');
const contactSheet = path.join(reviewDir, 'contact-sheet-cmu-jog-retarget-2026-07-12.png');
const actionName = 'jog-cmu-lower-body-audition';
const filePrefix = 'cmu-jog';
...
], 'Captured jog stride audit failed.');
...
run(blender, [
  ...
  '--sample-frames', '1,9,17,25',
], 'Captured jog review rendering failed.');
...
for (const python of pythonCandidates) {
  sheetResult = spawnSync(python, [
    ...
    '--output', contactSheet,
    '--file-prefix', filePrefix,
    '--title', 'CMU JOG RETARGET - CAPTURED GAIT + AUTHORED STICK CONTROL',
    '--frames', '1,9,17,25',
  ], { cwd: root, encoding: 'utf8', stdio: 'inherit' });
  if (!sheetResult.error && sheetResult.status === 0) break;
}
if (!sheetResult || sheetResult.error || sheetResult.status !== 0) {
  console.error(sheetResult?.error?.message ?? 'Captured jog contact sheet failed.');
...
for (const required of [outputBlend, retargetReport, strideReport, renderReport, contactSheet]) {
  if (!fs.existsSync(required)) {
    console.error(`Captured jog audition output is missing: ${required}`);
    process.exit(1);
  }
}
