...
const workfile = path.join(root, 'asset-inbox', 'players', 'vnext', 'goon-field-player-cmu-sprint-audition.blend');
...
const reportPath = path.join(root, 'asset-inbox', 'players', 'vnext', 'cmu-sprint-private-export-report.json');
fs.rmSync(reportPath, { force: true });
const result = spawnSync(blender, [
  '--background', workfile,
  '--python', path.join(root, 'scripts', 'blender', 'export_vnext_cmu_sprint_review.py'),
  '--', '--output-dir', outputDir, '--output-report', reportPath,
], { cwd: root, encoding: 'utf8', stdio: 'inherit' });
if (result.error || result.status !== 0 || !fs.existsSync(reportPath)) {
  console.error(result.error?.message ?? 'Blender captured-sprint review export failed.');
  process.exit(result.status ?? 1);
}

const report = JSON.parse(fs.readFileSync(reportPath, 'utf8'));
if (report.status !== 'private-runtime-review-exported' || report.publicRuntimeAllowed !== false) {
  console.error('Captured sprint assets did not retain their private-review boundary.');
  process.exit(1);
}
...
for (const side of ['home', 'away']) {
  ...
  const destination = path.join(runtimeDir, `field-${side}-cmu-sprint.glb`);
  fs.copyFileSync(source, destination);
  const sourceBytes = fs.readFileSync(source);
  const destinationBytes = fs.readFileSync(destination);
  const hash = (value) => crypto.createHash('sha256').update(value).digest('hex');
  if (hash(sourceBytes) !== hash(destinationBytes)) {
    console.error(`Private ${side} review asset failed byte-identity verification.`);
    process.exit(1);
  }
