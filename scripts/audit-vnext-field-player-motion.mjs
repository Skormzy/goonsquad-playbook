...
const report = path.join(root, 'asset-inbox', 'players', 'vnext', 'motion-quality-report.json');
fs.rmSync(report, { force: true });
const result = spawnSync(blender, [
  '--background', workfile,
  '--python', path.join(root, 'scripts', 'blender', 'audit_vnext_field_player_motion.py'),
  '--', '--output-report', report,
], { cwd: root, encoding: 'utf8', stdio: 'inherit' });
if (result.error || result.status !== 0 || !fs.existsSync(report)) {
  console.error(result.error?.message ?? 'Blender vNext motion audit failed.');
