...
const refinementReport = path.join(
  root, 'asset-inbox', 'players', 'vnext',
  'cmu16-ik-diagonal-stick-refinement-report.json',
);
const auditReport = path.join(
  root, 'asset-inbox', 'players', 'vnext',
  'cmu16-ik-diagonal-stick-audit.json',
);
const actionRenderReport = path.join(
  root, 'asset-inbox', 'players', 'vnext',
  'cmu16-ik-diagonal-stick-action-render-report.json',
);
const exportReport = path.join(
  root, 'asset-inbox', 'players', 'vnext',
  'cmu16-ik-diagonal-stick-private-export-report.json',
);
const evidenceDir = path.join(
  root, 'docs', 'vnext', 'evidence', 'athlete-diagonal-stick-review', 'actions',
);
...
if (!fs.existsSync(sourceWorkfile)) {
  console.error('The private natural-grip review workfile is missing.');
  process.exit(1);
}
...
run(blender, [
  ...
  '--python', path.join(root, 'scripts', 'blender', 'refine_vnext_diagonal_stick_controls.py'),
  '--',
  '--output-workfile', outputWorkfile,
  '--output-report', refinementReport,
], 'Diagonal-stick control authoring');
...
], 'Diagonal-stick upper-body audit');
...
], 'Diagonal-stick action review rendering');
...
if (!contactSheetBuilt) {
  console.error('Diagonal-stick contact-sheet generation failed.');
  process.exit(1);
}
...
run(blender, [
  ...
  '--output-tag', 'cmu16-ik-diagonal-stick',
], 'Diagonal-stick private GLB export');

for (const side of ['home', 'away']) {
  const source = path.join(
    privateOutputDir,
    `goon-field-player-${side}-cmu16-ik-diagonal-stick-review.glb`,
  );
  const target = path.join(
    root, 'src', 'assets', 'vnext3d-review',
    `field-${side}-cmu16-ik-diagonal-stick.glb`,
  );
  fs.copyFileSync(source, target);
}

console.log('GOON_DIAGONAL_STICK_REVIEW_READY');
