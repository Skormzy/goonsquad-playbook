...
def main():
    ...
    views = ['front', 'three-quarter', 'rear', 'upper-close', 'lower-close', 'stick-close']
    names = [f'{side}-{view}' for side in ('home', 'away') for view in views]
    images = [
        Image.open(input_dir / f'{name}.png').convert('RGB').resize((300, 375), Image.Resampling.LANCZOS)
        for name in names
    ]
