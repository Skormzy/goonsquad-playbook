...
const workfile = path.join(root, 'asset-inbox', 'players', 'vnext', 'goon-field-player-cmu16-run-audition.blend');
...
const reportPath = path.join(root, 'asset-inbox', 'players', 'vnext', 'cmu16-run-private-export-report.json');
...
const result = spawnSync(blender, [
  ...
  '--output-tag', 'cmu16-run',
], { cwd: root, encoding: 'utf8', stdio: 'inherit' });
if (result.error || result.status !== 0 || !fs.existsSync(reportPath)) {
  console.error(result.error?.message ?? 'CMU Subject 16 private runtime export failed.');
  process.exit(result.status ?? 1);
}

const report = JSON.parse(fs.readFileSync(reportPath, 'utf8'));
if (report.status !== 'private-runtime-review-exported' || report.publicRuntimeAllowed !== false) {
  console.error('CMU Subject 16 assets did not retain their private-review boundary.');
  process.exit(1);
}
...
for (const side of ['home', 'away']) {
  ...
  const destination = path.join(runtimeDir, `field-${side}-cmu16-run.glb`);
  fs.copyFileSync(source, destination);
  const sourceBytes = fs.readFileSync(source);
  const destinationBytes = fs.readFileSync(destination);
  const hash = (value) => crypto.createHash('sha256').update(value).digest('hex');
  if (hash(sourceBytes) !== hash(destinationBytes)) {
    console.error(`Private ${side} CMU Subject 16 asset failed byte-identity verification.`);
    process.exit(1);
  }
