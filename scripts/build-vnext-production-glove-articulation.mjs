...
const reports = {
  author: path.join(outputDirectory, 'production-glove-articulation-author-report.json'),
  audit: path.join(outputDirectory, 'production-glove-articulation-audit.json'),
...
if (!fs.existsSync(sourceFinish)) {
  console.error(`Required private finished glove source is missing: ${sourceFinish}`);
  process.exit(1);
}
...
runBlender(sourceFinish, 'refine_vnext_production_glove_articulation.py', [
  '--output-workfile', outputWorkfile,
  '--output-report', reports.author,
], 'private production glove articulation authoring');

runBlender(outputWorkfile, 'audit_vnext_production_glove_articulation.py', [
  '--author-report', reports.author,
  '--output-report', reports.audit,
], 'private production glove articulation audit');

runBlender(outputWorkfile, 'render_vnext_production_glove_articulation.py', [
  '--output-dir', closeDirectory,
  '--output-report', reports.close,
], 'private production glove articulation close review');
...
], 'private production glove articulation all-action review');

run('python', [
  path.join(root, 'scripts/create_vnext_production_glove_fit_sheets.py'),
  '--input-dir', closeDirectory,
  '--prefix', 'production-glove-articulation',
], 'private production glove articulation close contact sheets');

run('python', [
  path.join(root, 'scripts/create_vnext_upper_body_contact_sheets.py'),
  '--input-dir', actionDirectory,
], 'private production glove articulation action contact sheets');

console.log('GOON_VNEXT_PRODUCTION_GLOVE_ARTICULATION_READY');
