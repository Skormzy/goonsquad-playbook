...
const tag = 'cmu16-ik-face-pose';
const sourceWorkfile = path.join(
  root, 'asset-inbox', 'players', 'vnext',
  'goon-field-player-cmu16-ik-helmet-detail-audition.blend',
);
...
const evidenceRoot = path.join(
  root, 'docs', 'vnext', 'evidence', 'athlete-face-pose-review',
);
...
const reports = {
  ...
  face: path.join(root, 'asset-inbox', 'players', 'vnext', `${tag}-audit.json`),
...
if (!fs.existsSync(sourceWorkfile)) {
  console.error('The approved private open-face helmet source workfile is missing.');
  process.exit(1);
}
...
run(blender, [
  ...
  '--python', path.join(root, 'scripts', 'blender', 'refine_vnext_face_pose.py'),
  '--',
  '--output-workfile', outputWorkfile,
  '--output-report', reports.refinement,
], 'licensed eye and cornea restoration');

for (const [label, script, report] of [
  ['face pose audit', 'audit_vnext_face_pose.py', reports.face],
  ['face pose silhouette audit', 'audit_vnext_silhouette.py', reports.silhouette],
  ['face pose material audit', 'audit_vnext_materials.py', reports.materials],
  ['face pose topology audit', 'audit_vnext_uniform_topology.py', reports.topology],
  ['face pose deformation audit', 'audit_vnext_uniform_deformation.py', reports.deformation],
]) {
...
for (const side of ['home', 'away']) {
  ...
  run(process.execPath, [gltfTransformCli,
    ...
    '--texture-size', '1024',
  ], `${side} face transport optimization`);
  fs.copyFileSync(
    optimized,
...
], 'face runtime asset audit');

console.log('GOON_VNEXT_FACE_POSE_REVIEW_READY');
