...
const wordmark = path.join(root, 'src', 'assets', 'skull_logo.png');
const privateOutputDir = path.join(
  root, 'asset-inbox', 'players', 'vnext', 'private-runtime-review',
);
const evidenceDir = path.join(
  root, 'docs', 'vnext', 'evidence', 'athlete-silhouette-review', 'actions',
);
const reports = {
...
if (!fs.existsSync(sourceWorkfile) || !fs.existsSync(wordmark)) {
  console.error('The PBR source workfile or Goon Squad wordmark is missing.');
  process.exit(1);
}
...
run(blender, [
  ...
  '--python', path.join(root, 'scripts', 'blender', 'refine_vnext_silhouette_geometry.py'),
  '--',
...
run(blender, [
  ...
  '--python', path.join(root, 'scripts', 'blender', 'audit_vnext_silhouette.py'),
  '--', '--output-report', reports.silhouette,
], 'silhouette audit');
...
for (const side of ['home', 'away']) {
  ...
  run(process.execPath, [gltfTransformCli,
    ...
    '--texture-size', '1024',
  ], `${side} silhouette transport optimization`);
  fs.copyFileSync(
    optimized,
...
], 'silhouette runtime asset audit');

console.log('GOON_VNEXT_SILHOUETTE_REVIEW_READY');
