...
describe('view registry', () => {
  it('keeps internal tools outside the public registry', () => {
    expect(PUBLIC_VIEW_IDS).toEqual(['home', 'playbook', 'tactics', 'replay3d', 'strategy3d', 'playmaker', 'stats', 'profile', 'account']);
  ...
  it('makes rig review available to local development only', () => {
    ...
    expect(resolveInitialView('http://localhost/?view=rigreview')).toBe('home');
  ...
  it('resolves canonical content and viewing mode parameters', () => {
    expect(resolveInitialView('http://localhost/?content=home')).toBe('home');
  ...
  it('opens Squad Live from the bare product URL', () => {
    expect(resolveInitialView('http://localhost/')).toBe('home');
    expect(resolveInitialView('http://localhost/?unknown=1')).toBe('home');
  });
});
